...
class xdp_feeder
{
    public:
        xdp_feeder() = default;
        void xdp_setup(const char* addr)
        {
            ...
            ret = xsk_socket__create(&sock, addr, 0, umem, nullptr, &tx_ring, &cfg);
            if (ret) {
    ...
    private:
        inline void priv_send(Packet &pack)
        {
            uint32_t tx_idx;
            memcpy(xsk_umem__get_data(bufs, 0), &pack, sizeof(Packet));
